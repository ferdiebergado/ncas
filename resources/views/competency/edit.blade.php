...
@section('card-title', 'EDIT COMPETENCY')

@section('card-content')

<form action="{{ route('competencies.update', $competency) }}" method="POST" @if(app()->environment(['local',
    'testing']))
    novalidate
    @endif>
    @csrf
            ...
            <div class="field is-fullwidth">
                <p class="control">
                    <input type="text" class="input {{ $errors->has('title') ? 'is-danger':'' }}" id="title"
                        name="title" value="{{ old('title', $competency->title) }}" min="3" max="200"
                ...
                <div class="control">
                    <div id="select-level" class="select is-fullwidth {{ $errors->has('level_id') ? 'is-danger':'' }}">
                        <select id="level_id" name="level_id" required>
                            ...
                            <option value="{{ $key }}"
                                {{ old('level_id', $competency->level_id) && old('level_id', $competency->level_id) == $key ? 'selected':'' }}>
                                {{ $value }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="control">
                    <div id="select-category"
                        ...
                        <select id="category_id" name="category_id" required>
                            ...
                            <option value="{{ $key }}"
                                {{ old('category_id', $competency->category_id) && old('category_id', $competency->category_id) == $key ? 'selected':'' }}>
                                {{ $value }}
                            </option>
                            @endforeach
                        </select>
    ...
    <div class="field is-horizontal mb-3">
        ...
        <div class="field-body">
            <p class="control">
                ...
                <a class="button is-white ml-2" href="{{ route('competencies.index', $competency) }}">Close</a>
            </p>
        </div>
    </div>
</form>
